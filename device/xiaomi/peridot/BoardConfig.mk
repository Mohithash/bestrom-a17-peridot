...
# BestROM skip missing: -include device/xiaomi/peridot-miuicamera/BoardConfig.mk
...
# Kernel modules
first_stage_modules := $(strip $(shell cat $(TARGET_KERNEL_SOURCE)/modules.list.msm.pineapple $(DEVICE_PATH)/configs/modules/modules.list.first_stage))
second_stage_modules := $(strip $(shell cat $(DEVICE_PATH)/configs/modules/modules.list.second_stage))
vendor_dlkm_modules := $(strip $(shell cat $(DEVICE_PATH)/configs/modules/modules.list.vendor_dlkm))
system_dlkm_modules := $(strip $(shell sed 's/#.*$$//;/^$$/d;s|.*/||' $(DEVICE_PATH)/configs/modules/modules.list.system_dlkm))
...
# Partitions
# BestROM skip: include vendor/voltage/config/BoardConfigReservedSize.mk
...
# Sepolicy
# BestROM skip: include device/voltage/sepolicy/libperfmgr/sepolicy.mk
include device/qcom/sepolicy_vndr/SEPolicy.mk
# BestROM boot-first: disable incomplete device sepolicy
...
# BestROM ME-AGENT: use prebuilt Theettam Kernel 2.1 Image (GKI 6.1.175, boot-confirmed)
# Source: https://github.com/Mohithash/kernel_xiaomi_sm8635/releases/tag/v2.1
# Avoids Lineage kernel.mk (not present in pure AOSP BestROM tree)
THEETTAM_PREBUILT_KERNEL := true
TARGET_PREBUILT_KERNEL := $(DEVICE_PATH)/prebuilt/Image
TARGET_FORCE_PREBUILT_KERNEL := true
# Do not attempt in-tree kernel compile for bacon
TARGET_KERNEL_SOURCE :=
TARGET_KERNEL_CONFIG :=
TARGET_KERNEL_EXT_MODULE_ROOT :=
TARGET_KERNEL_EXT_MODULES :=

# BestROM: install Theettam prebuilt kernel rule
include device/xiaomi/peridot/theettam_kernel.mk

include device/xiaomi/peridot/theettam_dtb.mk
